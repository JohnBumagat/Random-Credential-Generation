...
class GeneratorApp(App):
    ...
    def build(self):
        ...
        # Main content area
        content = GridLayout(cols=1, spacing=5, size_hint=(1, 0.8))
        ...
        password_length.add_widget(Label(text="Length (8-25):",
                                         color=self.theme['text']))
        self.password_length_slider = Slider(min=8, max=25, value=12, step=1)
        ...
        self.result_layout = BoxLayout(
            orientation='vertical', spacing=100, size_hint=(1, 0.40))

        # Username result with copy button
        username_box = BoxLayout(size_hint_y=None, height=50)
        ...
        username_copy_btn = Button(text="Copy",
                                   ...
                                   background_color=self.theme['button'],
                                   color=self.theme['button_text'])
        username_copy_btn.bind(on_press=lambda x: self.copy_to_clipboard(
            self.username_result.text.replace("Username: ", "")))
        username_box.add_widget(self.username_result)
        username_box.add_widget(username_copy_btn)

        # Password result with copy button
        password_box = BoxLayout(size_hint_y=None, height=50)
        ...
        password_copy_btn = Button(text="Copy",
                                   ...
                                   background_color=self.theme['button'],
                                   color=self.theme['button_text'])
        password_copy_btn.bind(on_press=lambda x: self.copy_to_clipboard(
            self.password_result.text.replace("Password: ", "")))
    ...
    def generate_credentials(self, instance):
        ...
        username = self.generate_string(
            name, username_length, username_complexity)
    ...
    def apply_theme(self, color):
        ...
        bg_color = (max(0, color[0] * 0.2), max(0,
                    color[1] * 0.2), max(0, color[2] * 0.2), 1)
        input_color = (max(0, color[0] * 0.3), max(0,
                       color[1] * 0.3), max(0, color[2] * 0.3), 1)
        button_color = (
            max(0, color[0] * 0.4), max(0, color[1] * 0.4), max(0, color[2] * 0.4), 1)
